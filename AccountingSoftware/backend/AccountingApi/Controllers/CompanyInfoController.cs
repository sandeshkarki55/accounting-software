...
public class CompanyInfoController(Mediator mediator) : BaseController
{
    [HttpGet]
    ...
    {
    var result = await mediator.Send<PagedResult<CompanyInfoDto>>(new GetAllCompanyInfosQuery(pagination, sorting, filtering));
        return Ok(result);
    }
    ...
    {
    var companyInfo = await mediator.Send<CompanyInfoDto>(new CreateCompanyInfoCommand(createCompanyInfoDto));
    ...
    {
    var companyInfo = await mediator.Send<CompanyInfoDto>(new UpdateCompanyInfoCommand(id, updateCompanyInfoDto));
    ...
    {
    var result = await mediator.Send<bool>(new DeleteCompanyInfoCommand(id));
        if (!result)
    ...
    {
    var companyInfo = await mediator.Send<CompanyInfoDto>(new SetDefaultCompanyCommand(id));
        return Ok(companyInfo);
    }
}