...
interface AddCompanyModalProps {
  ...
  company?: CompanyInfo; // For editing
}

const AddCompanyModal: React.FC<AddCompanyModalProps> = ({ show, onHide, onCompanySaved, company }) => {
  const [formData, setFormData] = useState<CreateCompanyInfoDto>({
    companyName: '',
    legalName: '',
    taxNumber: '',
    registrationNumber: '',
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || `Failed to ${isEditing ? 'update' : 'create'} company`);
